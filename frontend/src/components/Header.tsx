...
function UserAvatar({ name }: { name?: string }) {
  const initials = (name || 'A')
    ...
    .slice(0, 2);
  return (
    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-black-700 to-black-900 text-xs font-bold text-white">
      {initials}
    </div>
  );
}
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex min-h-screen w-screen items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-lg">
        <h3 className="text-base font-semibold text-black-900">Keluar dari akun?</h3>
        <p className="mt-1.5 text-sm text-black-400">Kamu perlu masuk kembali untuk mengakses dashboard ini.</p>
        <div className="mt-5 flex justify-end gap-2">
          <button
            onClick={onCancel}
            className="rounded-lg border border-black-200 px-4 py-2 text-sm font-medium text-black-400 hover:bg-canvas"
          >
            Batal
          </button>
          <button
...
export default function Header({ user, onMenuClick, onLogout, onNewBooking }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-20 flex items-center justify-between border-b border-black-200 bg-white/80 px-4 py-4 backdrop-blur-sm sm:px-6">
      <div className="flex items-center gap-4">
        <button
          onClick={onMenuClick}
          className="rounded-lg border border-black-200 p-2 text-black-400 hover:bg-canvas lg:hidden"
        >
      ...
      <div className="flex items-center gap-3">
        {onNewBooking && (
          <button
            ...
            className="hidden items-center gap-2 rounded-lg bg-primary-500 px-4 py-2 text-sm font-medium text-white hover:bg-primary-600 sm:flex"
          >
            <Plus size={16} />
            Booking Baru
          </button>
            ...
            <div className="absolute right-0 top-full z-30 mt-2 w-80 overflow-hidden rounded-xl border border-black-200 bg-white shadow-xl sm:w-96">
              ...
              <div className="max-h-96 overflow-y-auto">
                {loadingNotifications ? (
                  <div className="py-8 text-center text-sm text-black-400">Memuat...</div>
                ) : notifications.length === 0 ? (
                  <div className="py-8 text-center text-sm text-black-400">Tidak ada notifikasi</div>
                ) : (
                  notifications.map((n) => {
            ...
            <>
              ...
              <div className="absolute right-0 top-full z-20 mt-2 w-48 rounded-lg border border-black-200 bg-white py-1 shadow-lg">
                <button
                  ...
                  className="flex w-full items-center gap-2 px-4 py-2 text-sm text-black-400 hover:bg-error-50 hover:text-error-500"
                >
                  <LogOut size={16} />
                  Logout
                </button>
