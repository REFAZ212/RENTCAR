      ...
      <div className="flex items-center justify-between">
        <div className="relative max-w-md w-full">
          ...
          <input type="text" placeholder="Cari nama garasi, pemilik, no HP..." value={search} onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-400 focus:border-brand-400 outline-none text-sm transition" />
        </div>
        <button onClick={() => { setForm(emptyPartnerForm); setEditItem(null); setShowForm(true); }}
          className="flex items-center gap-2 px-4 py-2 bg-brand-500 text-white text-sm font-medium rounded-lg hover:bg-brand-600 transition-colors shrink-0 ml-4">
          <Plus size={16} />
          Tambah Garasi
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-fade-in" onClick={() => { setShowForm(false); setEditItem(null); }}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
              <h2 className="font-display text-lg font-semibold text-ink-900">{editItem ? 'Edit Garasi Partner' : 'Tambah Garasi Partner'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={form.nama_garasi} onChange={(e) => setField('nama_garasi', e.target.value)} required
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-400 focus:border-brand-400 outline-none text-sm" />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea value={form.alamat} onChange={(e) => setField('alamat', e.target.value)} rows={2} required
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-400 focus:border-brand-400 outline-none text-sm resize-none" />
              </div>
              <div>
              ...
              <div className="flex items-center gap-3">
                <label className="relative inline-flex items-center cursor-pointer">
                  ...
                  <div className="w-9 h-5 bg-gray-200 peer-focus:ring-2 peer-focus:ring-brand-100 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-brand-500"></div>
                </label>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit" disabled={submitting}
                  className="px-4 py-2 text-sm font-medium bg-brand-500 text-white rounded-lg hover:bg-brand-600 disabled:opacity-50 transition-colors flex items-center gap-2">
      ...
      {showKendaraanForm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-fade-in" onClick={() => { setShowKendaraanForm(false); setKendaraanGarasi(null); setEditKendaraanItem(null); setKendaraanFotoFile(null); setKendaraanFotoPreview(null); }}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
              <div>
                <h2 className="font-display text-lg font-semibold text-ink-900">{editKendaraanItem ? 'Edit Kendaraan' : 'Tambah Kendaraan'}</h2>
            ...
            <form onSubmit={handleKendaraanSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={kendaraanForm.tipe_id || ''} onChange={(e) => setKendaraanField('tipe_id', e.target.value)}
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-400 focus:border-brand-400 outline-none text-sm disabled:bg-gray-100 disabled:text-gray-400">
                ...
                <div>
                  ...
                  <input type="text" value={kendaraanForm.plat_nomor || ''} onChange={(e) => setKendaraanField('plat_nomor', e.target.value)} required
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-400 focus:border-brand-400 outline-none text-sm font-mono uppercase" />
                </div>
                <div>
              ...
              <div>
                ...
                <div className="flex items-start gap-4">
                  <label className="flex-1 flex items-center justify-center px-4 py-6 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-brand-400 hover:bg-brand-50/50 transition-colors">
                    <div className="text-center">
                      <ImagePlus size={28} className="text-gray-400 mx-auto mb-1" />
                  ...
                  {kendaraanFotoPreview && (
                    <div className="relative shrink-0">
                      <img src={kendaraanFotoPreview} alt="Preview" className="w-24 h-24 object-cover rounded-xl border border-gray-200" />
                      <button type="button" onClick={() => { setKendaraanFotoFile(null); setKendaraanFotoPreview(null); }}
                        className="absolute -top-2 -right-2 w-5 h-5 bg-maint-500 text-white rounded-full flex items-center justify-center hover:bg-maint-600 transition-colors">
                        <X size={12} />
                      </button>
                    </div>
                  )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="text-left px-4 py-3 font-medium text-gray-500">Garasi</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500">Pemilik</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500">No. HP</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500">Kendaraan</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr><td colSpan={6} className="p-12 text-center">
                  <Loader2 size={24} className="text-brand-500 animate-spin mx-auto mb-2" />
                  <p className="text-sm text-gray-500">Memuat data...</p>
                </td></tr>
              ) : items.length === 0 ? (
                <tr><td colSpan={6} className="p-12 text-center">
                  <Building2 size={48} className="text-gray-300 mx-auto mb-3" strokeWidth={1.5} />
                  ...
                  <tr className="hover:bg-gray-50/50 transition-colors cursor-pointer" onClick={() => toggleExpand(item.id)}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <ChevronRight size={16} className={`text-gray-400 shrink-0 transition-transform ${expandedIds.has(item.id) ? 'rotate-90' : ''}`} />
                        <div>
                    ...
                    <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => handleEdit(item)} className="p-1.5 text-gray-400 hover:text-brand-600 hover:bg-brand-50 rounded-lg transition-colors" title="Edit">
                          <Pencil size={16} />
                        </button>
                        <button onClick={() => setConfirmDelete(item)} className="p-1.5 text-gray-400 hover:text-maint-500 hover:bg-maint-50 rounded-lg transition-colors" title="Hapus">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
                                  ...
                                  {(expandedKendaraans[item.id] || []).map((k) => (
                                    <tr key={k.id} className="hover:bg-gray-50/50 transition-colors">
                                      ...
                                      <td className="px-4 py-2.5 text-right">
                                        <div className="flex items-center justify-end gap-1">
                                          <button onClick={() => openEditKendaraan(item, k)} className="p-1.5 text-gray-400 hover:text-brand-600 hover:bg-brand-50 rounded-lg transition-colors" title="Edit">
                                            <Pencil size={16} />
                                          </button>
                                          <button onClick={() => setConfirmDeleteKendaraan(k)} className="p-1.5 text-gray-400 hover:text-maint-500 hover:bg-maint-50 rounded-lg transition-colors" title="Hapus">
                                            <Trash2 size={16} />
                                          </button>
                                        </div>
                                      </td>
    ...
    <div className="space-y-4">
          ...
          {editingName ? (
            <div className="flex items-center gap-1.5">
              <input
                ...
                autoFocus
                className="px-2 py-1 border border-brand-400 rounded-lg text-sm font-medium text-gray-900 focus:ring-2 focus:ring-brand-400 focus:border-brand-400 outline-none"
              />
              <button onClick={saveName} className="p-1 text-avail-500 hover:bg-avail-50 rounded-lg transition-colors" title="Simpan">
                <Check size={16} />
              </button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Tersedia', value: stats.tersedia, color: 'text-avail-500' },
          { label: 'Disewa', value: stats.disewa, color: 'text-rented-500' },
          { label: 'Servis', value: stats.maintenance, color: 'text-maint-500' },
        ].map((s) => (
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-fade-in" onClick={() => { setShowForm(false); setEditItem(null); }}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
              <h2 className="font-display text-lg font-semibold text-ink-900">{editItem ? 'Edit Kendaraan' : 'Tambah Kendaraan'}</h2>
              <button onClick={() => { setShowForm(false); setEditItem(null); }} className="p-1 hover:bg-gray-100 rounded-lg transition-colors">
                <X size={20} className="text-gray-400" />
              </button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              {lastAdded && !editItem && (
                <div className="flex items-center justify-between p-3 bg-avail-50 border border-avail-500/20 rounded-lg">
                  <div className="flex items-center gap-2">
                    <Check size={20} className="text-avail-500" />
                    <span className="text-sm font-medium text-avail-600">Kendaraan berhasil ditambahkan!</span>
                  </div>
                  <button type="button" onClick={() => setLastAdded(false)} className="text-sm font-medium text-avail-600 hover:text-avail-700 underline">Sembunyikan</button>
                </div>
              )}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                ...
                {editItem && (
                  <div>
                    ...
                    <select value={form.status} onChange={(e) => setField('status', e.target.value)} required
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-400 focus:border-brand-400 outline-none text-sm capitalize">
              ...
              <div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  {fotoPreview && (
                    <div className="relative shrink-0">
                      <img src={fotoPreview} alt="Preview" className="w-24 h-24 object-cover rounded-xl border border-gray-200" />
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                {lastAdded && !editItem ? (
                  <button type="button" onClick={() => setLastAdded(false)}
                    className="px-4 py-2 text-sm font-medium bg-avail-500 text-white rounded-lg hover:bg-avail-600 transition-colors flex items-center gap-2">
                    <Plus size={16} />
                    Tambah Lainnya
                  </button>
                ) : (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="text-left px-4 py-3 font-medium text-gray-500">Kendaraan</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500">Plat</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500">Kategori</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500">Harga/Hari</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500">Status</th>
                <th className="text-right px-4 py-3 font-medium text-gray-500">Aksi</th>
              </tr>
            </thead>
...
export default function GarasiPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-2xl font-semibold text-ink-900">Garasi</h1>
        ...
        {tabs.map((t) => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${
              activeTab === t ? 'bg-ink-900 text-white' : 'text-gray-500 hover:text-gray-700'
            }`}
          >
            {t}
          </button>
