import { useState, useEffect } from 'react';
import { useParams, Link, useSearchParams } from 'react-router-dom';
import {
  katalogAPI,
  type TipeKendaraan,
  type KategoriKendaraan,
  type GarasiPartner,
} from '../services/api';
import { formatRupiah, ADMIN_WA } from '../lib/format';

interface KatalogKendaraan {
  id: number | string;
  nama_kendaraan: string;
  tipe?: TipeKendaraan;
  merek: string;
  model: string;
  tahun: number;
  warna: string;
  plat_nomor: string;
  kapasitas_penumpang: number;
  harga_sewa_per_hari: number | string;
  foto?: string | null;
  catatan?: string | null;
  kategori?: KategoriKendaraan;
  garasi_partner?: GarasiPartner;
  status?: string;
  available_for_dates?: boolean;
  active_orders_count?: number;
  estimated_return_date?: string | null;
}

interface SpecItem {
  icon: string;
  label: string;
  value: string | number;
}

// Loosened shape used only to read an `id` off `tipe` / `kategori` for the recommendation
// lookup below. The real TipeKendaraan / KategoriKendaraan types (from services/api) are
// assumed to include an `id` field, as is standard for API resources — adjust here if not.
type WithId = { id?: number | string };

const getFotoUrl = (foto?: string | null): string | null => {
  if (!foto) return null;
  if (foto.startsWith('http')) return foto;
  return `/storage/${foto}`;
};

const formatTanggal = (dateStr: string): string =>
  new Date(dateStr).toLocaleDateString('id-ID', {
    day: 'numeric',
    month: 'long',
    year: 'numeric',
  });

const buildWALink = (item: KatalogKendaraan, adminPhone = ADMIN_WA): string => {
  const pesan = `Halo, saya tertarik untuk menyewa:\n\n${item.nama_kendaraan} (${item.merek} ${item.model} ${item.tahun})\nPlat: ${item.plat_nomor}\nHarga: ${formatRupiah(item.harga_sewa_per_hari)}/hari\n\nTanggal: -\nDurasi: - hari\n\nMohon info ketersediaan dan cara pemesanannya. Terima kasih.`;
  return `https://wa.me/${adminPhone}?text=${encodeURIComponent(pesan)}`;
};

function DetailSkeleton() {
  return (
    <div className="min-h-screen bg-canvas">
      <div className="max-w-4xl mx-auto px-4 py-10">
        <div className="skeleton h-6 w-32 mb-6 rounded" />
        <div className="skeleton h-80 w-full rounded-xl mb-6" />
        <div className="bg-white rounded-2xl border border-black-200 p-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div className="space-y-3">
              <div className="skeleton h-6 w-24 rounded-full" />
              <div className="skeleton h-8 w-64 rounded" />
              <div className="skeleton h-5 w-40 rounded" />
            </div>
            <div className="text-right space-y-2">
              <div className="skeleton h-8 w-32 rounded" />
              <div className="skeleton h-4 w-16 rounded ml-auto" />
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-accent-100">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="flex items-center gap-3">
                <div className="skeleton h-9 w-9 rounded-lg" />
                <div className="space-y-1">
                  <div className="skeleton h-3 w-12 rounded" />
                  <div className="skeleton h-4 w-20 rounded" />
                </div>
              </div>
            ))}
          </div>
        </div>
        <div className="skeleton h-32 w-full rounded-2xl" />
      </div>
    </div>
  );
}

function NotFoundView() {
  return (
    <div className="min-h-screen bg-canvas flex items-center justify-center">
      <div className="text-center">
        <svg className="w-16 h-16 text-black-200 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={1.5}
            d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
          />
        </svg>
        <h2 className="text-xl font-bold text-black-700 mb-2">Kendaraan Tidak Ditemukan</h2>
        <p className="text-black-400 mb-4">Kendaraan mungkin sudah tidak tersedia</p>
        <Link
          to="/katalog"
          className="px-5 py-2.5 bg-primary-600 text-white text-sm font-medium rounded-lg hover:bg-primary-700 transition-colors inline-block"
        >
          Kembali ke Katalog
        </Link>
      </div>
    </div>
  );
}

/* --- KARTU REKOMENDASI --- */
function RecommendationCard({ item }: { item: KatalogKendaraan }) {
  const fotoUrl = getFotoUrl(item.foto);
  return (
    <Link
      to={`/katalog/${item.id}`}
      className="group bg-white rounded-xl border border-black-200 overflow-hidden hover:shadow-lg hover:border-primary-200 transition-all duration-200"
    >
      <div className="h-32 bg-accent-100 overflow-hidden">
        {fotoUrl ? (
          <img
            src={fotoUrl}
            alt={item.nama_kendaraan}
            loading="lazy"
            className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
          />
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            <svg className="w-8 h-8 text-black-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 17h.01M16 17h.01M3 11l1.5-5A2 2 0 016.4 4h11.2a2 2 0 011.9 1.4L21 11M3 11h18M3 11v6a1 1 0 001 1h1a1 1 0 001-1v-1h12v1a1 1 0 001 1h1a1 1 0 001-1v-6" />
            </svg>
          </div>
        )}
      </div>
      <div className="p-3">
        <h4 className="text-sm font-semibold text-black-900 line-clamp-1 group-hover:text-primary-600 transition-colors">
          {item.nama_kendaraan}
        </h4>
        <p className="text-xs text-black-400 mt-0.5">
          {item.merek} {item.model} &middot; {item.tahun}
        </p>
        <p className="text-sm font-bold text-primary-600 mt-1.5">
          {formatRupiah(item.harga_sewa_per_hari)}
          <span className="text-[11px] text-black-400 font-normal">/hari</span>
        </p>
      </div>
    </Link>
  );
}

export default function KendaraanDetail() {
  const { id } = useParams<{ id: string }>();
  const [searchParams] = useSearchParams();
  const [item, setItem] = useState<KatalogKendaraan | null>(null);
  const [loading, setLoading] = useState(true);
  const [notFound, setNotFound] = useState(false);
  const [recommendations, setRecommendations] = useState<KatalogKendaraan[]>([]);

  const tanggalMulai = searchParams.get('tanggal_mulai') || '';
  const durasiHari = searchParams.get('durasi_hari') || '';

  useEffect(() => {
    if (!id) {
      setNotFound(true);
      setLoading(false);
      return;
    }

    setLoading(true);
    setNotFound(false);
    setRecommendations([]);

    const params: Record<string, string> = {};
    if (tanggalMulai) params.tanggal_mulai = tanggalMulai;
    if (durasiHari) params.durasi_hari = durasiHari;

    katalogAPI
      .get(Number(id), Object.keys(params).length > 0 ? params : undefined)
      .then(({ data }) => setItem(data as unknown as KatalogKendaraan))
      .catch(() => setNotFound(true))
      .finally(() => setLoading(false));
  }, [id, tanggalMulai, durasiHari]);

  // Kendaraan sedang disewa / servis / tidak tersedia untuk tanggal dipilih ->
  // cari kendaraan lain dengan tipe yang sama (fallback: kategori yang sama) yang masih tersedia.
  useEffect(() => {
    if (!item) return;
    const sedangDisewa = item.status === 'disewa';
    const sedangServis = item.status === 'maintenance';
    const tidakTersediaTanggal = tanggalMulai !== '' && item.available_for_dates === false;
    if (!sedangDisewa && !sedangServis && !tidakTersediaTanggal) return;

    const tipeId = (item.tipe as unknown as WithId | undefined)?.id;
    const kategoriId = (item.kategori as unknown as WithId | undefined)?.id;

    katalogAPI
      .list({ page: 1, sort: 'terbaru' })
      .then(({ data }) => {
        const list = (data.data ?? []) as unknown as KatalogKendaraan[];
        const isAvailable = (v: KatalogKendaraan) =>
          String(v.id) !== String(item.id) &&
          v.status !== 'disewa' &&
          v.status !== 'maintenance' &&
          v.available_for_dates !== false;

        let pool = tipeId
          ? list.filter((v) => isAvailable(v) && (v.tipe as unknown as WithId | undefined)?.id === tipeId)
          : [];
        if (pool.length === 0 && kategoriId) {
          pool = list.filter((v) => isAvailable(v) && (v.kategori as unknown as WithId | undefined)?.id === kategoriId);
        }
        setRecommendations(pool.slice(0, 4));
      })
      .catch(() => setRecommendations([]));
  }, [item, tanggalMulai]);

  if (loading) return <DetailSkeleton />;
  if (notFound || !item) return <NotFoundView />;

  const specs: SpecItem[] = [
    {
      icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4',
      label: 'Tipe',
      value: item.tipe?.nama_tipe?.toUpperCase() ?? '-',
    },
    {
      icon: 'M8 17h.01M16 17h.01M3 11l1.5-5A2 2 0 016.4 4h11.2a2 2 0 011.9 1.4L21 11M3 11h18M3 11v6a1 1 0 001 1h1a1 1 0 001-1v-1h12v1a1 1 0 001 1h1a1 1 0 001-1v-6',
      label: 'Kapasitas',
      value: `${item.kapasitas_penumpang} kursi`,
    },
    {
      icon: 'M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01',
      label: 'Warna',
      value: item.warna,
    },
    {
      icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z',
      label: 'Plat',
      value: item.plat_nomor,
    },
  ];

  const fotoUrl = getFotoUrl(item.foto);
  const waLink = buildWALink(item);

  const sedangDisewa = item.status === 'disewa';
  const sedangServis = item.status === 'maintenance';
  const tidakTersediaTanggal = tanggalMulai !== '' && item.available_for_dates === false;
  const isUnavailable = sedangDisewa || sedangServis || tidakTersediaTanggal;

  const statusLabel = sedangServis
    ? 'Sedang Servis'
    : sedangDisewa
      ? 'Sedang Disewa'
      : tidakTersediaTanggal
        ? 'Tidak Tersedia'
        : 'Tersedia';

  const statusBadgeClass = sedangDisewa
    ? 'text-error-600 bg-error-50'
    : sedangServis || tidakTersediaTanggal
      ? 'text-accent-600 bg-accent-50'
      : 'text-success-600 bg-success-50';

  const statusDotClass = sedangDisewa
    ? 'bg-error-500'
    : sedangServis || tidakTersediaTanggal
      ? 'bg-accent-500'
      : 'bg-success-500';

  return (
    <div className="min-h-screen bg-canvas">
      {/* Top bar */}
      <nav className="bg-white border-b border-black-200 sticky top-0 z-40" aria-label="Navigasi detail kendaraan">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14">
          <Link to="/katalog" className="flex items-center gap-2 text-sm text-black-400 hover:text-primary-600 transition-colors">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
            Katalog
          </Link>
          <a
            href={waLink}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 px-4 py-2 bg-success-500 text-white text-sm font-medium rounded-lg hover:bg-success-600 transition-colors"
          >
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
            </svg>
            Pesan via WhatsApp
          </a>
        </div>
      </nav>

      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        {/* Foto */}
        <article className="relative bg-white rounded-2xl border border-black-200 overflow-hidden mb-6 shadow-sm">
          {fotoUrl ? (
            <img
              src={fotoUrl}
              alt={`${item.merek} ${item.model}`}
              className="w-full h-72 sm:h-96 object-cover"
            />
          ) : (
            <div className="w-full h-72 sm:h-96 bg-accent-100 flex items-center justify-center">
              <svg className="w-20 h-20 text-black-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={1.5}
                  d="M8 17h.01M16 17h.01M3 11l1.5-5A2 2 0 016.4 4h11.2a2 2 0 011.9 1.4L21 11M3 11h18M3 11v6a1 1 0 001 1h1a1 1 0 001-1v-1h12v1a1 1 0 001 1h1a1 1 0 001-1v-6"
                />
              </svg>
            </div>
          )}
          {isUnavailable && (
            <div className="absolute inset-0 bg-black/25 flex items-end p-4">
              <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold text-white shadow-lg ${statusDotClass}`}>
                <span className="w-1.5 h-1.5 rounded-full bg-white" />
                {statusLabel}
              </span>
            </div>
          )}
        </article>

        {/* Info */}
        <section className="bg-white rounded-2xl border border-black-200 p-6 mb-6" aria-label="Detail kendaraan">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                {item.tipe && (
                  <span className="text-xs font-semibold text-primary-600 bg-primary-50 px-2.5 py-1 rounded-full uppercase">
                    {item.tipe.nama_tipe}
                  </span>
                )}
                <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${statusBadgeClass}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${statusDotClass}`} />
                  {statusLabel}
                </span>
              </div>
              <h1 className="text-2xl font-bold text-black-900 mt-2">{item.nama_kendaraan}</h1>
              <p className="text-black-400 mt-1">
                {item.merek} {item.model} &middot; {item.tahun}
              </p>
            </div>
            <div className="text-right shrink-0">
              <div className="text-2xl font-bold text-primary-600">{formatRupiah(item.harga_sewa_per_hari)}</div>
              <div className="text-sm text-black-400">per hari</div>
            </div>
          </div>

          {/* Specs grid */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-accent-100">
            {specs.map((s, i) => (
              <div key={i} className="flex items-center gap-3">
                <div className="w-9 h-9 bg-canvas rounded-lg flex items-center justify-center shrink-0">
                  <svg className="w-4 h-4 text-black-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={s.icon} />
                  </svg>
                </div>
                <div>
                  <div className="text-xs text-black-400">{s.label}</div>
                  <div className="text-sm font-semibold text-black-900 capitalize">{s.value}</div>
                </div>
              </div>
            ))}
          </div>

          {/* Garasi */}
          {item.garasi_partner && (
            <div className="mt-4 pt-4 border-t border-accent-100 flex items-center gap-2 text-sm text-black-400">
              <svg className="w-4 h-4 text-black-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
              </svg>
              Garasi: <span className="font-medium text-black-900">{item.garasi_partner.nama_garasi}</span>
            </div>
          )}

          {/* Catatan */}
          {item.catatan && (
            <div className="mt-4 pt-4 border-t border-accent-100">
              <div className="text-xs text-black-400 mb-1">Catatan</div>
              <p className="text-sm text-black-700 whitespace-pre-line">{item.catatan}</p>
            </div>
          )}
        </section>

        {/* Status banner: Sedang Servis */}
        {sedangServis && (
          <div className="bg-accent-50 border border-accent-200 rounded-2xl p-5 mb-6 flex items-start gap-3">
            <div className="w-9 h-9 bg-accent-100 rounded-lg flex items-center justify-center shrink-0 mt-0.5">
              <svg className="w-5 h-5 text-accent-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
              </svg>
            </div>
            <div>
              <p className="text-sm font-semibold text-accent-700">Kendaraan sedang dalam perawatan/servis</p>
              {item.estimated_return_date && (
                <p className="text-xs text-accent-600 mt-1">
                  Perkiraan siap: {formatTanggal(item.estimated_return_date)}
                </p>
              )}
              <p className="text-xs text-accent-600 mt-1">
                Hubungi kami untuk info kendaraan lain yang tersedia sekarang.
              </p>
            </div>
          </div>
        )}

        {/* Status banner: Sedang Disewa */}
        {sedangDisewa && (
          <div className="bg-error-50 border border-error-200 rounded-2xl p-5 mb-6 flex items-start gap-3">
            <div className="w-9 h-9 bg-error-100 rounded-lg flex items-center justify-center shrink-0 mt-0.5">
              <svg className="w-5 h-5 text-error-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <div>
              <p className="text-sm font-semibold text-error-700">Kendaraan ini sedang disewa</p>
              {item.estimated_return_date && (
                <p className="text-xs text-error-600 mt-1">
                  Perkiraan kembali: {formatTanggal(item.estimated_return_date)}
                </p>
              )}
              <p className="text-xs text-error-600 mt-1">
                Hubungi kami untuk mengetahui ketersediaan kendaraan ini.
              </p>
            </div>
          </div>
        )}

        {/* Status banner: Tidak tersedia untuk tanggal yang dipilih */}
        {tidakTersediaTanggal && !sedangDisewa && !sedangServis && (
          <div className="bg-accent-50 border border-accent-200 rounded-2xl p-5 mb-6 flex items-start gap-3">
            <div className="w-9 h-9 bg-accent-100 rounded-lg flex items-center justify-center shrink-0 mt-0.5">
              <svg className="w-5 h-5 text-accent-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
              </svg>
            </div>
            <div>
              <p className="text-sm font-semibold text-accent-700">Kendaraan tidak tersedia untuk tanggal ini</p>
              <p className="text-xs text-accent-600 mt-1">
                Kendaraan ini sudah memiliki order pada tanggal yang Anda pilih. Silakan pilih tanggal lain atau lihat rekomendasi di bawah.
              </p>
              <Link
                to="/katalog"
                className="inline-flex items-center gap-1 text-xs font-medium text-accent-600 hover:text-accent-700 mt-2 underline underline-offset-2"
              >
                Lihat katalog lengkap
              </Link>
            </div>
          </div>
        )}

        {/* Rekomendasi kendaraan lain */}
        {isUnavailable && recommendations.length > 0 && (
          <section className="mb-6">
            <div className="flex items-center gap-2 mb-3">
              <svg className="w-4 h-4 text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
              </svg>
              <h3 className="text-base font-bold text-black-900">Kendaraan Lain yang Bisa Anda Pilih</h3>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {recommendations.map((rec) => (
                <RecommendationCard key={rec.id} item={rec} />
              ))}
            </div>
          </section>
        )}

        {/* CTA */}
        <section
          className={`rounded-2xl p-6 text-white ${
            isUnavailable
              ? 'bg-gradient-to-br from-black-700 to-black-900'
              : 'bg-gradient-to-br from-accent-500 to-accent-600'
          }`}
        >
          <h2 className="text-lg font-bold">
            {isUnavailable ? 'Butuh Bantuan Mencari Kendaraan Lain?' : 'Tertarik dengan kendaraan ini?'}
          </h2>
          <p className="text-white/80 text-sm mt-1">
            {isUnavailable
              ? 'Tim kami siap membantu carikan kendaraan yang tersedia sesuai kebutuhan Anda'
              : 'Hubungi kami via WhatsApp untuk konsultasi dan pemesanan'}
          </p>
          <a
            href={waLink}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-4 inline-flex items-center gap-2 px-6 py-3 bg-white text-black-900 font-semibold rounded-xl hover:bg-canvas transition-colors shadow-lg"
          >
            <svg className="w-5 h-5 text-success-500" viewBox="0 0 24 24" fill="currentColor">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
            </svg>
            Pesan via WhatsApp
          </a>
        </section>
      </main>
    </div>
  );
}