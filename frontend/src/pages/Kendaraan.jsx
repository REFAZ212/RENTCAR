...
export default function Kendaraan() {
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => {
    garasiPartnerAPI.list()
      .then(({ data }) => setGarasi(data.data))
      .catch(() => {});
    kategoriAPI.list()
  ...
  const handleStatusChange = async (id, status) => {
    try {
      await kendaraanAPI.update(id, { status });
      toast.success(`Status kendaraan diubah menjadi "${statusLabels[status]}"`);
      load();
    } catch {
      toast.error('Gagal mengubah status kendaraan');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setForm(emptyForm); setEditItem(null); setFotoFile(null); setFotoPreview(null); setLastAdded(false); setShowForm(true); }}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={form.garasi_partner_id} onChange={(e) => setField('garasi_partner_id', e.target.value)} required
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm">
                    <option value="">Pilih Garasi</option>
                    {garasi.map((g) => <option key={g.id} value={g.id}>{g.nama_garasi}</option>)}
                  </select>
                </div>
                <div>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-700">Rp {Number(item.harga_sewa_per_hari).toLocaleString('id-ID')}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusStyles[item.status]}`}>{statusLabels[item.status]}</span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      {item.status === 'tersedia' && (
                        <button onClick={() => setConfirmAction({ title: 'Masuk ke Maintenance', message: `Ubah status "${item.nama_kendaraan}" menjadi Maintenance? Kendaraan tidak akan bisa disewa selama dalam status ini.`, onConfirm: () => handleStatusChange(item.id, 'maintenance') })}
                          className="px-2.5 py-1 text-xs font-medium bg-yellow-100 text-yellow-700 rounded-lg hover:bg-yellow-200 transition-colors" title="Maintenance">
                          Maintenance
                        </button>
                      )}
                      {item.status === 'maintenance' && (
                        <button onClick={() => setConfirmAction({ title: 'Aktifkan Kendaraan', message: `Keluarkan "${item.nama_kendaraan}" dari Maintenance? Kendaraan akan menjadi Tersedia.`, onConfirm: () => handleStatusChange(item.id, 'tersedia') })}
                          className="px-2.5 py-1 text-xs font-medium bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center gap-1" title="Aktifkan">
                          <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                          Aktifkan
                        </button>
                      )}
                      {item.status === 'disewa' && (
                        <span className="px-2.5 py-1 text-xs font-medium text-gray-400 italic">Sedang disewa</span>
                      )}
