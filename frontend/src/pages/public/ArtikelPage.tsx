      ...
      <section className="bg-ink-950 py-20 sm:py-28">
        <div className="max-w-[1400px] mx-auto px-5 sm:px-8 lg:px-20">
          <AnimatedSection>
            ...
            <p className="mt-5 text-ink-400 text-[15px] leading-relaxed max-w-xl">Tips, panduan, dan insight seputar transportasi dan mobilitas.</p>
          </AnimatedSection>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {artikel.map((a, i) => (
            <AnimatedSection key={a.title} delay={i * 0.06}>
              <article className="group bg-white rounded-[20px] border border-gray-100 overflow-hidden transition-all duration-200 hover:-translate-y-1 hover:shadow-lg hover:shadow-black/5">
                <div className="aspect-[16/9] bg-gray-100 flex items-center justify-center">
                  <BookOpen size={32} className="text-gray-200" />
                </div>
                <div className="p-6">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="px-2 py-0.5 bg-gray-100 text-[10px] font-semibold text-ink-500 rounded">{a.category}</span>
                    <span className="text-[11px] text-ink-400">{a.date}</span>
                  </div>
                  <h3 className="font-display font-semibold text-[15px] text-ink-950 group-hover:text-brand-600 transition-colors">{a.title}</h3>
                  <p className="text-sm text-ink-400 mt-2 leading-relaxed">{a.excerpt}</p>
                </div>
              </article>
            </AnimatedSection>
          ))}
