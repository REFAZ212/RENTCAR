    ...
    <div className="min-h-screen">
      <section className="bg-ink-950 py-20 sm:py-28">
        <div className="max-w-[1400px] mx-auto px-5 sm:px-8 lg:px-20">
          <AnimatedSection>
            <span className="text-[11px] font-semibold tracking-widest uppercase text-brand-400 mb-4 block">Karir</span>
      ...
      <section className="max-w-[1400px] mx-auto px-5 sm:px-8 lg:px-20 py-20 sm:py-28">
        <AnimatedSection className="max-w-xl mb-14">
          <span className="text-[11px] font-semibold tracking-widest uppercase text-brand-500 mb-4 block">Mengapa PILAR</span>
          <h2 className="font-display text-2xl sm:text-3xl font-bold text-ink-950 tracking-tight">Lingkungan Kerja Terbaik</h2>
        </AnimatedSection>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[
            ...
            <AnimatedSection key={item.title} delay={i * 0.06}>
              <div className="p-6 rounded-[20px] border border-gray-100 bg-white hover:shadow-lg hover:shadow-black/5 transition-all duration-200">
                <h3 className="font-semibold text-ink-950 text-[15px]">{item.title}</h3>
                <p className="mt-2 text-ink-400 text-sm leading-relaxed">{item.desc}</p>
              </div>
            </AnimatedSection>
          ))}
        </div>
      ...
      <section className="bg-gray-50 py-20 sm:py-28">
        <div className="max-w-[1400px] mx-auto px-5 sm:px-8 lg:px-20">
          <AnimatedSection className="max-w-xl mb-14">
            <span className="text-[11px] font-semibold tracking-widest uppercase text-brand-500 mb-4 block">Lowongan</span>
            <h2 className="font-display text-2xl sm:text-3xl font-bold text-ink-950 tracking-tight">Posisi Yang Tersedia</h2>
          </AnimatedSection>
          <div className="space-y-4">
            {lowongan.map((l, i) => (
              <AnimatedSection key={l.title} delay={i * 0.06}>
                <div className="bg-white rounded-[20px] border border-gray-100 p-7 flex flex-col sm:flex-row sm:items-center gap-5 hover:shadow-lg hover:shadow-black/5 transition-all duration-200">
                  <div className="w-12 h-12 bg-gray-50 rounded-xl flex items-center justify-center shrink-0">
                    <Briefcase size={20} className="text-ink-950" strokeWidth={1.5} />
                  </div>
                  <div className="flex-1">
                    <h3 className="font-display font-semibold text-[15px] text-ink-950">{l.title}</h3>
                    <p className="text-ink-400 text-sm mt-1 leading-relaxed">{l.desc}</p>
                    <div className="flex items-center gap-4 mt-3">
                      <span className="flex items-center gap-1.5 text-[12px] text-ink-400"><MapPin size={12} />{l.location}</span>
                      <span className="flex items-center gap-1.5 text-[12px] text-ink-400"><Clock size={12} />{l.type}</span>
                    </div>
                  </div>
                  <a
