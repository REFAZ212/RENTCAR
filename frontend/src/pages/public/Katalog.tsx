...
export default function Katalog() {
  ...
  const [kategoriSlug, setKategoriSlug] = useState(searchParams.get('kategori') ?? '');
  const [tipeSlug, setTipeSlug] = useState(searchParams.get('tipe') ?? '');
  const [sort, setSort] = useState(() => {
    const s = searchParams.get('sort');
    return SORT_OPTIONS.some((o) => o.value === s) ? (s as string) : 'terbaru';
  });
  ...
  const initialDurasi = parseInt(searchParams.get('durasi_hari') ?? '', 10);
  const [tanggalMulai, setTanggalMulai] = useState(
    initialTanggalMulai && initialTanggalMulai >= catalogToday ? initialTanggalMulai : ''
  );
  const [durasiHari, setDurasiHari] = useState(Number.isFinite(initialDurasi) && initialDurasi > 0 ? initialDurasi : 1);
  ...
  useEffect(() => {
    ...
    if (kategoriSlug) params.kategori = kategoriSlug;
    if (tipeSlug) params.tipe = tipeSlug;
    if (sort !== 'terbaru') params.sort = sort;
    if (tanggalMulai) {
      params.tanggal_mulai = tanggalMulai;
      params.durasi_hari = String(durasiHari);
    }
    if (page > 1) params.page = String(page);
    setSearchParams(params, { replace: true });
  }, [search, kategoriSlug, tipeSlug, sort, tanggalMulai, durasiHari, page, setSearchParams]);

  useEffect(() => {
    if (loading) return;
    const raw = sessionStorage.getItem('katalog_return');
    if (!raw) return;
    sessionStorage.removeItem('katalog_return');
    let itemId: number | undefined;
    let scrollY: number | undefined;
    try {
      const target = JSON.parse(raw) as { itemId?: number; scrollY?: number };
    ...
    const apply = () => {
      if (itemId) {
        const el = document.getElementById(`vehicle-${itemId}`);
        if (el) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setHighlightItemId(null), 2500);
  ...
  const hasActiveFilters = search !== '' || kategoriSlug !== '' || tipeSlug !== '' || tanggalMulai !== '' || sort !== 'terbaru';
  ...
  return (
    <div className="bg-canvas">
      ...
      <section className="bg-black">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-16">
          <AnimatedSection>
            ...
            {meta?.jam_operasional && meta.jam_operasional.length > 0 && (
              <div className="mt-6 inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-full text-sm text-black-400">
                ...
                <span>Jam operasional: <span className="text-white/90">{formatJamOperasional(meta.jam_operasional)}</span></span>
              </div>
            )}
          </AnimatedSection>
        </div>
      ...
      {/* Katalog */}
      <section id="katalog" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 scroll-mt-20">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-bold text-black">Katalog Kendaraan</h2>
            <p className="text-sm text-black-400 mt-1">
              {loading ? 'Memuat...' : `${meta?.total ?? 0} kendaraan ditemukan`}
              {tanggalMulai && meta ? ` untuk ${durasiHari} hari` : ''}
            </p>
          </div>
          {hasActiveFilters && (
            <button
              onClick={handleResetFilters}
              className="inline-flex items-center justify-center gap-1.5 self-start sm:self-center px-3.5 py-2 text-sm font-medium border border-black-200 text-black-600 rounded-xl hover:border-error-500/40 hover:text-error-600 hover:bg-error-50/40 transition-colors"
            >
        ...
        <div className="bg-white rounded-xl border border-black-200 p-4 mb-6 flex flex-col sm:flex-row gap-3">
          ...
          <select
            ...
            className="w-full sm:w-auto px-3 py-2 border border-black-200 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 outline-none"
            aria-label="Urutkan"
          >
            {SORT_OPTIONS.map((s) => (
        ...
        {/* Kategori Tabs */}
        {kategoris.length > 0 && (
          <div className="flex gap-2 mb-4 overflow-x-auto pb-2 no-scrollbar" role="tablist" aria-label="Filter kategori">
            <button
        ...
        {/* Tipe Tabs */}
        {tipes.length > 0 && (
          <div className="flex gap-2 mb-6 overflow-x-auto pb-2 no-scrollbar" role="tablist" aria-label="Filter tipe">
            <button
              onClick={() => setTipeSlug('')}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {items.map((item) => (
                <VehicleCard
                  ...
                  durasiHari={tanggalMulai ? durasiHari : undefined}
                  isHighlighted={highlightItemId === item.id}
                />
              ))}
            </div>
            {showPagination && (
              <nav className="flex items-center justify-center gap-1.5 mt-8" aria-label="Paginasi">
                <button
                ...
                {pageNumbers.map((p, idx) => (
                  <Fragment key={p}>
                    {idx > 0 && p - pageNumbers[idx - 1] > 1 && (
                      <span className="px-1 text-sm text-black-400">…</span>
                    )}
                    <button
                      onClick={() => setPage(p)}
                      disabled={p === page}
                      aria-current={p === page ? 'page' : undefined}
                      className={`min-w-9 h-9 px-2 text-sm font-semibold rounded-lg border transition-colors ${
                        p === page
                          ? 'bg-primary-600 text-white border-primary-600'
                          : 'border-black-200 text-black-600 hover:border-primary-300 hover:text-primary-600'
                      }`}
                    >
                      {p}
                    </button>
      ...
      {debouncedTanggal && unavailableCount > 0 && serupaItems.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-10">
          <div className="bg-gradient-to-r from-primary-50 to-primary-100 rounded-2xl border border-primary-100 p-6">
      ...
      {modalItem && (
        <PesanSekarangModal
          ...
          initialTanggalMulai={tanggalMulai || undefined}
          initialDurasi={tanggalMulai ? durasiHari : undefined}
        />
      )}
    </div>
  );
