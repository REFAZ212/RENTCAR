    ...
    >
      ...
      >
        <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between rounded-t-2xl z-10">
          <div>
            <h2 className="text-lg font-bold text-ink-950">Pesan Sekarang</h2>
            <p className="text-sm text-ink-400">{item.nama_kendaraan}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-5">
          {success ? (
            <div className="text-center py-6">
              ...
              <h3 className="text-xl font-bold text-ink-950 mb-2">Pesanan Terkirim!</h3>
            ...
            <>
              <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl mb-5">
                ...
                <div className="min-w-0">
                  <p className="font-bold text-ink-950 text-sm line-clamp-1">{item.nama_kendaraan}</p>
                  <p className="text-xs text-ink-400">{item.merek} {item.model} &middot; {item.tahun}</p>
                  <p className="text-sm font-bold text-brand-600 mt-0.5">{formatRupiah(item.harga_sewa_per_hari)}/hari</p>
                </div>
              </div>
              {error && (
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-shadow"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-ink-700 mb-1">
                  ...
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-ink-400">+62</span>
                    <input
                      type="tel"
                      value={form.no_hp}
                      onChange={(e) => handleChange('no_hp', e.target.value.replace(/[^0-9]/g, ''))}
                      placeholder="8xxx"
                      className="w-full pl-12 pr-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-shadow"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm font-medium text-ink-700 mb-1">
                      Jam Mulai <span className="text-ink-400 font-normal">(opsional)</span>
                    </label>
                    <input
                      type="time"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-ink-700 mb-1">
                      Jam Selesai <span className="text-ink-400 font-normal">(opsional)</span>
                    </label>
                    <input
                      type="time"
                      ...
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-shadow disabled:bg-gray-50 disabled:text-ink-400"
                    />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-ink-700 mb-2">Opsi Supir</label>
                  <div className="flex gap-4">
                    <label className={`flex items-center gap-2.5 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all flex-1 ${
                      ...
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${
                        form.opsi_supir === 'lepas_kunci' ? 'border-brand-500' : 'border-gray-300'
                      }`}>
                        {form.opsi_supir === 'lepas_kunci' && <div className="w-2 h-2 rounded-full bg-brand-500" />}
                      </div>
                      <div>
                        <div className="text-sm font-medium text-ink-950">Lepas Kunci</div>
                        <div className="text-xs text-ink-400">Tanpa supir</div>
                      </div>
                    </label>
                    <label className={`flex items-center gap-2.5 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all flex-1 ${
                      ...
                      />
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${
                        form.opsi_supir === 'dengan_supir' ? 'border-brand-500' : 'border-gray-300'
                      }`}>
                        {form.opsi_supir === 'dengan_supir' && <div className="w-2 h-2 rounded-full bg-brand-500" />}
                      </div>
                      <div>
                        <div className="text-sm font-medium text-ink-950">Dengan Supir</div>
                        <div className="text-xs text-ink-400">Biaya diinfo admin</div>
                      </div>
                    </label>
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-ink-700 mb-1">Catatan (opsional)</label>
                  <textarea
                    value={form.catatan}
                    onChange={(e) => handleChange('catatan', e.target.value)}
                    placeholder="Contoh: butuh antar ke hotel, dll"
                    rows={2}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-shadow resize-none"
                  />
                </div>
                <div className="bg-brand-50 rounded-xl p-4">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm text-ink-500">Harga/hari</span>
                    <span className="text-sm font-medium text-ink-950">{formatRupiah(item.harga_sewa_per_hari)}</span>
                  </div>
                  {durasiHari > 0 && (
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm text-ink-500">Durasi</span>
                      <span className="text-sm font-medium text-ink-950">{durasiHari} hari</span>
                    </div>
                  )}
                  {form.opsi_supir === 'dengan_supir' && (
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm text-ink-500">Supir</span>
                      <span className="text-xs text-orange-600 font-medium">Biaya diinfo admin</span>
                    </div>
                  )}
                  <div className="border-t border-brand-200 pt-2 flex justify-between items-center">
                    <span className="text-sm font-semibold text-ink-950">Perkiraan Total</span>
              ...
              <div className="mt-6 space-y-3">
                <button
                  ...
                  disabled={submitting}
                  className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-brand-600 text-white font-semibold rounded-xl hover:bg-brand-700 disabled:opacity-50 transition-colors shadow-lg"
                >
                  {submitting ? (
                    <>
    ...
    <div
      ...
      className={`group bg-white rounded-xl border overflow-hidden transition-all duration-200 ${
        isDisabled
          ...
          : 'border-gray-200 hover:shadow-lg hover:border-brand-200 cursor-pointer'
      }`}
    >
      <div className="relative h-44 bg-gray-100 overflow-hidden">
        ...
        {item.tipe && (
          <span className="absolute top-3 left-3 px-2.5 py-1 bg-white/90 backdrop-blur-sm text-xs font-semibold text-ink-700 rounded-lg uppercase">
            {item.tipe.nama_tipe}
          </span>
        )}
        {isDisabled && (
      ...
      <div className="p-4">
        <h3 className={`font-bold text-ink-950 transition-colors line-clamp-1 ${
          isDisabled ? '' : 'group-hover:text-brand-600'
        }`}>
          {item.nama_kendaraan}
        </h3>
        ...
        <div className="mt-3 pt-3 border-t border-gray-100">
          <div className="flex items-center justify-between mb-3">
            <div>
              <span className="text-lg font-bold text-brand-600">{formatRupiah(item.harga_sewa_per_hari)}</span>
              <span className="text-xs text-ink-400">/hari</span>
            </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-40" role="navigation" aria-label="Navigasi utama">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
          <Link to="/" className="flex items-center gap-2.5">
            ...
            <span className="text-lg font-bold text-ink-950 hidden sm:block">PILAR Rental</span>
          </Link>
          <div className="flex items-center gap-3">
            <Link
              to="/"
              className="flex items-center gap-2 px-4 py-2 bg-gray-100 text-ink-700 text-sm font-medium rounded-lg hover:bg-gray-200 transition-colors"
            >
              Beranda
            </Link>
            <a
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-16">
          <AnimatedSection>
            <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white leading-tight tracking-tight">
              Sewa Kendaraan
              <br />
              <span className="text-brand-400">Mudah &amp; Terpercaya</span>
            </h1>
            <p className="mt-4 text-ink-400 text-lg leading-relaxed max-w-2xl">
            ...
            <div className="mt-6 flex flex-wrap gap-4">
              <a
                href="#katalog"
                className="px-6 py-3 bg-brand-600 text-white font-semibold rounded-xl hover:bg-brand-700 transition-colors shadow-lg shadow-brand-600/25"
              >
                Lihat Katalog
              </a>
              <a
      ...
      <section className="bg-white border-b border-gray-200" aria-label="Statistik layanan">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 text-center">
            {[
              { label: 'Unit Tersedia', value: `${statusCounts.tersedia}`, color: 'text-emerald-600' },
              { label: 'Sedang Disewa', value: `${statusCounts.disewa}`, color: 'text-red-500' },
              { label: 'Sedang Servis', value: `${statusCounts.servis}`, color: 'text-amber-500' },
              { label: 'Total Unit', value: `${meta?.total ?? 0}`, color: 'text-brand-600' },
            ].map((s, i) => (
      ...
      <section id="katalog" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-bold text-ink-950">Katalog Kendaraan</h2>
            <p className="text-sm text-ink-400 mt-1">Pilih kendaraan yang sesuai kebutuhan Anda</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6 flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-shadow"
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6">
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2 shrink-0">
              ...
              <span className="text-sm font-medium text-ink-700">Cek Ketersediaan:</span>
            </div>
            <input
              type="date"
              value={tanggalMulai}
            ...
            <div className="flex items-center gap-1.5">
              <input
                ...
                className="w-20 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 outline-none"
                aria-label="Durasi hari"
              />
              <span className="text-sm text-ink-400">hari</span>
            </div>
            {tanggalMulai && (
              <button
          ...
          <div className="flex gap-2 mb-4 overflow-x-auto pb-2" role="tablist" aria-label="Filter kategori">
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-full whitespace-nowrap transition-colors ${
                kategoriSlug === '' ? 'bg-brand-600 text-white shadow-sm' : 'bg-white text-ink-600 border border-gray-200 hover:border-brand-300 hover:text-brand-600'
              }`}
              role="tab"
            ...
            {kategoris.map((k) => (
              <button
                ...
                className={`px-4 py-2 text-sm font-medium rounded-full whitespace-nowrap transition-colors ${
                  kategoriSlug === k.slug ? 'bg-brand-600 text-white shadow-sm' : 'bg-white text-ink-600 border border-gray-200 hover:border-brand-300 hover:text-brand-600'
                }`}
                role="tab"
              ...
              >
                ...
                {k.kendaraans_count > 0 && (
                  <span className={`ml-1.5 text-xs ${kategoriSlug === k.slug ? 'text-brand-200' : 'text-ink-400'}`}>
                    ({k.kendaraans_count})
                  </span>
                )}
              </button>
          ...
          <div className="flex gap-2 mb-6 overflow-x-auto pb-2" role="tablist" aria-label="Filter tipe">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-full whitespace-nowrap transition-colors ${
                tipeSlug === '' ? 'bg-purple-600 text-white shadow-sm' : 'bg-white text-ink-600 border border-gray-200 hover:border-purple-300 hover:text-purple-600'
              }`}
              role="tab"
            ...
            {tipes.map((t) => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-medium rounded-full whitespace-nowrap transition-colors ${
                  tipeSlug === t.slug ? 'bg-purple-600 text-white shadow-sm' : 'bg-white text-ink-600 border border-gray-200 hover:border-purple-300 hover:text-purple-600'
                }`}
                role="tab"
              ...
              >
                ...
                {t.kendaraans_count > 0 && (
                  <span className={`ml-1 ${tipeSlug === t.slug ? 'text-purple-200' : 'text-ink-400'}`}>
                    ({t.kendaraans_count})
                  </span>
                )}
              </button>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-ink-500 font-medium">Tidak ada kendaraan ditemukan</p>
            <p className="text-sm text-ink-400 mt-1">Coba ubah filter pencarian Anda</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
      ...
      {debouncedTanggal && unavailableCount > 0 && serupaItems.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-10">
          <div className="bg-gradient-to-r from-brand-50 to-indigo-50 rounded-2xl border border-brand-100 p-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h3 className="text-lg font-bold text-ink-950">Kendaraan Serupa yang Tersedia</h3>
      ...
      <section className="bg-brand-600">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 text-center">
          ...
          <p className="mt-2 text-brand-100">Hubungi kami via WhatsApp untuk konsultasi dan pemesanan cepat</p>
          <a
      ...
      <footer className="bg-ink-950 text-ink-400">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <div className="mt-8 pt-6 border-t border-ink-800 text-center text-sm">
