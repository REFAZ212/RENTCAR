        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {promos.map((p, i) => (
            <AnimatedSection key={p.title} delay={i * 0.06}>
              <div className="bg-white rounded-[24px] border border-accent-100 p-8 hover:shadow-lg hover:shadow-black/5 transition-all duration-200 h-full flex flex-col">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 bg-primary-50 rounded-xl flex items-center justify-center">
                    <Tag size={18} className="text-primary-600" />
                  </div>
                  <span className="px-2.5 py-1 bg-primary-50 text-[11px] font-bold text-primary-600 rounded-md">{p.code}</span>
                </div>
                <h3 className="font-display text-lg font-bold text-black">{p.title}</h3>
                <p className="mt-2 text-black-400 text-sm leading-relaxed flex-1">{p.desc}</p>
                <div className="mt-6 flex items-center justify-between">
                  <span className="text-[11px] text-black-400">Berlaku hingga {p.validUntil}</span>
                  <a
