    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Kelola kategori dan tipe kendaraan dalam satu tempat</p>
        </div>
          ...
          <div className="divide-y divide-gray-100">
            {filtered.map((kategori) => {
              ...
              return (
                <div key={kategori.id}>
                  <div
                    className={`flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-gray-50/50 transition-colors ${isExpanded ? 'bg-blue-50/30' : ''}`}
                    onClick={() => toggleExpand(kategori.id)}
                  >
                    <svg className={`w-4 h-4 text-gray-400 shrink-0 transition-transform duration-200 ${isExpanded ? 'rotate-90' : ''}`}
                    ...
                    <div className="w-9 h-9 bg-blue-50 rounded-lg flex items-center justify-center shrink-0">
                      <svg className="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>
                    </div>

                    <div className="flex-1 min-w-0">
                      <div className="font-medium text-gray-900 text-sm">{kategori.nama_kategori}</div>
                      <div className="text-xs text-gray-500 truncate">{kategori.deskripsi || kategori.slug}</div>
                    </div>
                    ...
                    <span className={`px-2 py-1 text-xs font-medium rounded-full shrink-0 ${kategori.aktif ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500'}`}>
                    ...
                    <div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button onClick={() => { setConfirmDelete(kategori); setDeleteType('kategori'); }} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
