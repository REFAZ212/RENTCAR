  ...
  return (
    <div>
      ...
      <div className="flex items-start gap-3">
        <label className="flex-1 flex items-center justify-center gap-2 px-3 py-2 border-2 border-dashed border-black-200 rounded-lg cursor-pointer hover:border-primary-400 hover:bg-primary-50/50 transition-colors">
        ...
        {(preview || existing) && (
          <div className="relative shrink-0">
            ...
            {existing && !preview && <span className="absolute -top-1 -right-1 w-4 h-4 bg-success-500 rounded-full flex items-center justify-center"><svg className="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg></span>}
          </div>
        )}
      </div>
    </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-black-200 p-4">
        <div className="relative max-w-md">
          ...
          <input type="text" placeholder={`Cari nama atau no HP ${isSupir ? 'supir' : 'calo'}...`} value={search} onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-10 pr-4 py-2 border border-black-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none text-sm transition" />
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={form.nama} onChange={(e) => setField('nama', e.target.value)} required
                    className="w-full px-3 py-2 border border-black-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none text-sm" />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea value={form.alamat} onChange={(e) => setField('alamat', e.target.value)} rows={2}
                  className="w-full px-3 py-2 border border-black-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none text-sm resize-none" />
              </div>
              <div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-fade-in" onClick={() => setDetailItem(null)}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-black-200 flex items-center justify-between sticky top-0 bg-white z-10">
              ...
              <button onClick={() => setDetailItem(null)} className="p-1 hover:bg-canvas rounded-lg transition-colors">
                <svg className="w-5 h-5 text-black-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium mt-1 ${
                    detailItem.status === 'active' ? 'bg-primary-50 text-primary-500' : 'bg-black-100 text-black-400'
                  }`}>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {items.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-xl shadow-sm border border-black-200 hover:shadow-md hover:border-primary-200 transition-all overflow-hidden flex flex-col cursor-pointer"
            ...
            >
              <div className="p-4 pb-3 flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium mt-1 ${
                    item.status === 'active' ? 'bg-primary-50 text-primary-500' : 'bg-black-100 text-black-400'
                  }`}>
              ...
              <div className="p-4 pt-3 mt-3 border-t border-black-200 flex items-center gap-2">
                <a
                  ...
                  className="flex items-center justify-center gap-1.5 flex-1 px-3 py-1.5 bg-success-50 text-success-600 text-xs font-medium rounded-lg hover:bg-success-100 transition-colors"
                >
