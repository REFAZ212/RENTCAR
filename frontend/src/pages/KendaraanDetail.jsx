...
function buildWALink(item, adminPhone = '62895361054272') {
...
export default function KendaraanDetail() {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/katalog" className="px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
            Kembali ke Katalog
          </Link>
        </div>
      </div>
  ...
  const specs = [
    { icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4', label: 'Tipe', value: item.tipe?.toUpperCase() },
    { icon: 'M8 17h.01M16 17h.01M3 11l1.5-5A2 2 0 016.4 4h11.2a2 2 0 011.9 1.4L21 11M3 11h18M3 11v6a1 1 0 001 1h1a1 1 0 001-1v-1h12v1a1 1 0 001 1h1a1 1 0 001-1v-6', label: 'Kapasitas', value: `${item.kapasitas_penumpang} kursi` },
    { icon: 'M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01', label: 'Warna', value: item.warna },
    { icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', label: 'Plat', value: item.plat_nomor },
  ];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14">
          ...
          <a href={buildWALink(item)} target="_blank" rel="noopener noreferrer"
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden mb-6">
          {item.foto ? (
            <img src={getFotoUrl(item.foto)} alt={item.nama_kendaraan} className="w-full h-72 sm:h-96 object-cover" />
          ) : (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              <span className="text-xs font-semibold text-blue-700 bg-blue-50 px-2.5 py-1 rounded-full uppercase">{item.tipe}</span>
              <h1 className="text-2xl font-bold text-gray-900 mt-2">{item.nama_kendaraan}</h1>
              <p className="text-gray-500 mt-1">{item.merek} {item.model} &middot; {item.tahun}</p>
            </div>
            ...
            {specs.map((s, i) => (
              <div key={i} className="flex items-center gap-3">
                <div className="w-9 h-9 bg-gray-50 rounded-lg flex items-center justify-center shrink-0">
                  <svg className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={s.icon} /></svg>
                </div>
                <div>
