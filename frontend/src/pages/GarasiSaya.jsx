...
export default function GarasiSaya() {
  ...
  const startEditNama = () => {
    setNamaValue(garasi?.nama_garasi || '');
    setEditingNama(true);
  };
  ...
  const saveNama = async () => {
    const trimmed = namaValue.trim();
    if (!trimmed || trimmed === garasi.nama_garasi) { setEditingNama(false); return; }
    setSavingNama(true);
    try {
      const { data } = await garasiPartnerAPI.update(garasi.id, { nama_garasi: trimmed });
      setGarasi((prev) => ({ ...prev, nama_garasi: data.nama_garasi }));
      toast.success('Nama garasi berhasil diperbarui');
      setEditingNama(false);
    } catch (err) {
      toast.error(err.response?.data?.message || 'Gagal memperbarui nama garasi');
    } finally {
      setSavingNama(false);
    }
  };
  ...
  if (!garasi) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-sm text-gray-400 mt-1">Tandai garasi sebagai milik sendiri dari tabel Garasi Partner</p>
      </div>
    );
  }
          ...
          {editingNama ? (
            <div className="flex items-center gap-2 mt-1">
              <input
                ...
                onKeyDown={(e) => { if (e.key === 'Enter') saveNama(); if (e.key === 'Escape') cancelEditNama(); }}
                autoFocus
                className="px-2 py-0.5 border border-blue-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
              />
              <button onClick={saveNama} disabled={savingNama}
              ...
              <button onClick={cancelEditNama}
                className="p-1 text-gray-400 hover:bg-gray-100 rounded-lg transition-colors">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
          ) : (
            <button onClick={startEditNama}
              className="group flex items-center gap-1.5 mt-1 text-sm text-gray-500 hover:text-blue-600 transition-colors">
              <span>{garasi.nama_garasi}</span>
                ...
                {editItem && (
                  <div>
                    ...
                    <select value={form.status} onChange={(e) => setField('status', e.target.value)} required
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm capitalize">
