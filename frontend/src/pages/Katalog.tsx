...
// Base URL API — sesuaikan lewat file .env (VITE_API_BASE_URL), fallback ke domain production
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'https://api.udinrentcar.com';
...
function VehicleCard({
  ...
  const isRented = !isUnavailable && item.status === 'disewa' && (item.active_orders_count ?? 0) > 0;
  ...
  return (
    ...
    >
      <div className="relative h-44 bg-gray-100 overflow-hidden">
        {fotoUrl ? (
          <img
            ...
            className={`w-full h-full object-cover transition-transform duration-300 ${
              isUnavailable || isRented ? '' : 'group-hover:scale-105'
            }`}
          />
        ) : (
          <div className="w-full h-full flex items-center justify-center">
        ...
        {isRented && (
          <div className="absolute inset-0 bg-black/10 flex flex-col items-center justify-center gap-1.5">
            <span className="px-3 py-1.5 bg-amber-500 text-white text-xs font-bold rounded-full shadow-lg">
              Sedang Disewa
            </span>
            {item.estimated_return_date && (
              <span className="text-[11px] text-white/90 font-medium bg-black/30 px-2 py-0.5 rounded-full">
                Kembali: {new Date(item.estimated_return_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
              </span>
            )}
          </div>
        )}
      ...
      <div className="p-4">
        <h3 className={`font-bold text-gray-900 transition-colors line-clamp-1 ${
          isUnavailable || isRented ? '' : 'group-hover:text-blue-600'
        }`}>
          {item.nama_kendaraan}
        </h3>
        ...
        <div className="mt-3 pt-3 border-t border-gray-100">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${
              ...
              {isUnavailable ? 'Tidak Tersedia' : isRented ? 'Sedang Disewa' : 'Tersedia'}
            </span>
          </div>
          <button
            onClick={(e) => {
              ...
              if (isUnavailable) return;
              if (isRented) {
                window.open(`https://wa.me/${ADMIN_WA}?text=${encodeURIComponent(`Halo, saya ingin menanyakan ketersediaan ${item.nama_kendaraan} (${item.merek} ${item.model}). Mohon info kapan kendaraan ini bisa disewa. Terima kasih.`)}`, '_blank');
                return;
              }
              onPesan(item);
            }}
          ...
          >
            ...
            {isUnavailable ? 'Tidak Tersedia' : isRented ? 'Hubungi untuk Ketersediaan' : 'Pesan Sekarang'}
          </button>
        </div>
      </div>
    </div>
