  ...
  if (!garasi) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-sm text-black-400 mt-1">Tandai garasi di menu Garasi Partner dengan centang "Milik Sendiri"</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-black-900">Garasi Saya</h1>
          <p className="text-sm text-black-400 mt-1">{garasi.nama_garasi}</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl shadow-sm border border-black-200 p-4">
          <div className="text-2xl font-bold text-black-900">{stats.total}</div>
          <div className="text-sm text-black-400">Total Kendaraan</div>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-black-200 p-4">
          <div className="text-2xl font-bold text-accent-600">{stats.tersedia}</div>
          <div className="text-sm text-black-400">Tersedia</div>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-black-200 p-4">
          <div className="text-2xl font-bold text-primary-600">{stats.disewa}</div>
          <div className="text-sm text-black-400">Disewa</div>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-black-200 p-4">
      ...
      {/* Search + Filter */}
      <div className="bg-white rounded-xl shadow-sm border border-black-200 p-4 space-y-3">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              {lastAdded && !editItem && (
                <div className="flex items-center justify-between p-3 bg-accent-50 border border-accent-200 rounded-lg">
                  <div className="flex items-center gap-2">
                    <svg className="w-5 h-5 text-accent-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                    <span className="text-sm font-medium text-accent-700">Kendaraan berhasil ditambahkan!</span>
                  </div>
                  <button type="button" onClick={() => setLastAdded(false)} className="text-sm font-medium text-accent-600 hover:text-accent-700 underline">Sembunyikan</button>
                </div>
              )}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={form.plat_nomor} onChange={(e) => setField('plat_nomor', e.target.value)} required
                    className="w-full px-3 py-2 border border-black-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm font-mono uppercase" />
                </div>
                <div>
              ...
              <div>
                ...
                <div className="flex items-start gap-4">
                  <label className="flex-1 flex items-center justify-center px-4 py-6 border-2 border-dashed border-black-200 rounded-lg cursor-pointer hover:border-blue-400 hover:bg-primary-50/50 transition-colors">
                    <div className="text-center">
                      <svg className="w-8 h-8 text-black-400 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                  ...
                  {fotoPreview && (
                    <div className="relative shrink-0">
                      <img src={fotoPreview} alt="Preview" className="w-24 h-24 object-cover rounded-lg border border-black-200" />
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-accent-100">
                ...
                {lastAdded && !editItem ? (
                  <button type="button" onClick={() => setLastAdded(false)}
                    className="px-4 py-2 text-sm font-medium bg-accent-600 text-white rounded-lg hover:bg-accent-600 transition-colors flex items-center gap-2">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
          ...
          <table className="w-full text-sm">
            <thead className="bg-canvas border-b border-black-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-black-400">Tipe</th>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-12 text-center">
                    ...
                    <p className="text-sm text-black-400 mt-1">Mulai dengan menambahkan kendaraan baru</p>
                  </td>
                </tr>
              ) : filtered.map((item) => (
                <tr key={item.id} className="hover:bg-canvas/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button onClick={() => setConfirmDelete(item)} className="p-1.5 text-black-400 hover:text-error-600 hover:bg-error-50 rounded-lg transition-colors" title="Hapus">
